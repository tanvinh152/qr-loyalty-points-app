...
type SheetContentProps = React.ComponentProps<typeof SheetPrimitive.Content> &
  HTMLMotionProps<"div"> & {
    side?: Side
    /** Renameable so a wrapper can own the slot name. See SheetContent. */
    "data-slot"?: string
  }
...
function SheetContent({
  ...
  style,
  children,
  // Pulled out so a wrapper can rename the slot. Everything else in `props`
  // goes to Radix's Content, which merges it onto this child via `asChild` —
  // but Radix's Slot lets the CHILD win, so a `data-slot` passed in from
  // outside would lose to the literal below. ui/drawer.tsx renames it to
  // `drawer-content`, which account-menu.test.tsx queries by name.
