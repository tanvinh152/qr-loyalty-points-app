 ...
 * Both rules fire on UPSTREAM code in this vendored file, and both are scoped
 * here rather than fixed so the tree stays diffable against the registry.
 *
 * `react-hooks/refs` (4 hits): the handlers below are built with
 * composeEventHandlers(childProps.onX, () => startAnimation(...)). The rule
 * sees a ref-reading callback passed as an argument and assumes it may run
 * during render; composeEventHandlers only STORES it and returns a new
 * function, so every ref read happens at event time.
 *
 * `react-hooks/set-state-in-effect` (2 hits): the `animate` prop effect and the
 * animateOnView effect deliberately push a prop/viewport transition into state.
 * Both are guarded by an early return, so they settle in one extra render
