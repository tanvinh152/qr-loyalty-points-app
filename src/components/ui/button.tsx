...
// Full class strings, so Tailwind sees them (an interpolated variant compiles
// to nothing — the same reason STAGGER and ENGAGEMENT_SPAN are lookups).
const COARSE_SM =
...
// `relative` is here for the small sizes' hit-box expansion below: on a
// coarse pointer a `::before` inset outward makes a 32px button answer to a
// 44px thumb without drawing or reflowing anything. Raising the heights instead
// would have reflowed every admin table row.
const buttonVariants = cva(
  "group/button relative inline-flex shrink-0 items-center justify-center gap-2 rounded-md border border-transparent font-semibold whitespace-nowrap duration-instant ease-out-quart transition-colors outline-none select-none focus-visible:ring-2 focus-visible:ring-primary/30 disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      // Touch minimum is 44px. `xs`/`icon-xs` are 24px and `sm`/`icon-sm`
      // 32px, so on a coarse pointer each grows an invisible hit box to 44:
      // 24 + 2×10, 32 + 2×6. Visual size and layout are untouched.
      size: {
        default: "h-10 px-4 text-body-sm",
        xs: `h-6 px-2 text-label-md [&_svg:not([class*='size-'])]:size-3 ${COARSE_XS}`,
        sm: `h-8 px-3 text-label-md [&_svg:not([class*='size-'])]:size-3.5 ${COARSE_SM}`,
        lg: "h-11 px-5 text-body-sm",
        // Full-bleed form CTA (`py-4` in claim-page.html).
        xl: "h-14 w-full px-6 text-body-lg",
        icon: "size-10",
        "icon-xs": `size-6 [&_svg:not([class*='size-'])]:size-3 ${COARSE_XS}`,
        "icon-sm": `size-8 [&_svg:not([class*='size-'])]:size-3.5 ${COARSE_SM}`,
        "icon-lg": "size-11",
      },
    },
    defaultVariants: {
