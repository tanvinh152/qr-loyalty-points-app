...
export function useAutoHeight<T extends HTMLElement = HTMLDivElement>(
  ...
  React.useLayoutEffect(() => {
    if (height === 0) {
      const next = measure()
      // Measuring the DOM is the textbook case the set-state-in-effect rule
      // carves out: the layout is an external system, there is no render-time
      // way to read it, and the guard above means this runs once rather than
      // cascading. Left as upstream wrote it.
